...
export default function AdminDashboardPage() {
  ...
  const fetchStudios = async () => {
    const res = await fetch("/api/admin/studios/list");
  ...
  const fetchUsers = async () => {
    ...
    if (!error && profiles) setUsers(profiles as Profile[]);
  };
  ...
  const handleDeleteStudio = async (studioId: string) => {
    const res = await fetch("/api/admin/studios/delete", {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  if (isLoading) {
    return <div className="p-8 text-center">Loading superadmin dashboard...</div>;
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        {/* Create Studio + User */}
        <div>
          <h2 className="text-xl font-semibold mb-2">Create New Studio + User</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input value={studioDomain} onChange={(e) => setStudioDomain(e.target.value)} />
            </div>
            <div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {studios.map((studio) => (
              <li key={studio.id} className="flex justify-between items-center border p-3 rounded-md">
                <div>
                  <p className="font-medium">{studio.name}</p>
                  <p className="text-sm text-gray-500">{studio.domain}</p>
                </div>
                <Button variant="destructive" onClick={() => handleDeleteStudio(studio.id)}>
                  Delete
                </Button>
              </li>
            ))}
